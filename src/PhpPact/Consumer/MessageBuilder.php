...
class MessageBuilder extends AbstractBuilder
{
    ...
    public function given(string $description, array $params = []): self
    {
        if (\count($params) > 0) {
            foreach ($params as $name => $value) {
                $this->ffi->pactffi_message_given_with_param($this->message, $description, $name, $value);
            }
        } else {
     ...
     * @return self
     */
    public function withContent(string $contents, ?string $contentType = null): self
    {
        $length = \strlen($contents);
        $size   = $length + 1;
        $cData  = $this->ffi->new("uint8_t[{$size}]");
        FFI::memcpy($cData, $contents, $length);
        $this->ffi->pactffi_message_with_contents($this->message, $contentType, $cData, $size);
