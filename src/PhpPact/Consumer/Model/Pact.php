...
class Pact extends AbstractPact
{
    ...
    public function __construct(private MockServerConfigInterface $config)
    {
        ...
        $this->id = $this->ffi->pactffi_new_pact($config->getConsumer(), $config->getProvider());
        $supportedVersions = [
        ...
        $version = $supportedVersions[$config->getPactSpecificationVersion()] ?? $this->ffi->PactSpecification_Unknown;
    ...
    {
        $interaction->setId($this->ffi->pactffi_new_interaction($this->id, $interaction->getDescription()));
        $this
            ->given($interaction)
